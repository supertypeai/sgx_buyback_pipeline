...
def extract_table_data(table_element: str) -> dict[str, str | list[str]]:
    table_data = {}
    ...
    for row in rows:
        ...
        values = [v for v in (c.get_text(strip=True) for c in cells[1:]) if v]
...
def get_sgx_announcements(url: str): 
    try:
        ...
        announcements_section = extract_section_data(soup, "Announcement Details")
        ...
        announcement_reference = announcements_section.get('Announcement Reference', '')
        ...
        # Get purchase date 
        purchase_date_raw = section_a.get('Date of Purchase', '')
        purchase_date = safe_extract_value(purchase_date_raw)
        purchase_date = safe_convert_datetime(purchase_date)

        # Get price per share
        price_paid_per_share = section_a.get('Price Paid per share', '')
        highest_per_share = section_a.get('Highest Price per share', '') 
        lowest_per_share = section_a.get('Lowest Price per share', '')
        ...
        total_share_purchased_raw = section_a.get('Total Number of shares purchased', '') 
        ...
        total_consideration_raw = section_a.get('Total Consideration', '') 
...
# def run_payload_sgx_announcements(sgx_payload: SGXAnnouncement)
...
if __name__ == '__main__':
    ...
    result = get_sgx_announcements(test_url2)
