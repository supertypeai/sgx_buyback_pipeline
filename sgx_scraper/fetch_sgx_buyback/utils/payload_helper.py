...
def build_price_per_share(
    ...
    for price_value in price_values:
        if price_value:
            currency_match = re.match(r"([A-Z]{3})\s", price_value)
    ...
    if price_paid_per_share:
        price_per_share["price_paid_per_share"] = safe_convert_float(
            price_paid_per_share
        )

    elif highest_per_share and lowest_per_share:
        price_per_share["highest"] = safe_convert_float(highest_per_share)
        price_per_share["lowest"] = safe_convert_float(lowest_per_share)

    if currency:
        price_per_share["currency"] = currency
...
def compute_mandate_remaining(
    total_mandate: float, 
    cumulative_purchased: float
) -> float | None:
...
def clean_payload_sgx_buyback(payload: list[dict[str, any]]) -> list[dict[str, any]]:
    ...
    for row in payload:
        ...
        if price_per_share:
            price_per_share.pop("currency", None)
